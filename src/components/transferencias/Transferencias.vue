...
const loadTasks = async () => {
  try {
    const response = await axios.get('users/' + userStore.userId + '/tasks')
...
<template>
  ...
  <hr>
  <div v-if="!onlyCurrentTasks" class="mb-3 d-flex justify-content-between flex-wrap">
    ...
    <div class="mx-2 mt-2">
      <button type="button" class="btn btn-success px-4 btn-addtask" @click="addTask"><i
          class="bi bi-xs bi-plus-circle"></i>&nbsp; Add Task</button>
    </div>
  </div>
  <task-table :tasks="filteredTasks" :showId="true" :showOwner="false" @edit="editTask"
